...
- Destructive scheduling actions ask for confirmation. A closure conflict explains that existing bookings/holds are protected. Cancellation copy states that no refund is issued automatically and that downstream updates are pending. Failed availability checks disable scheduling controls.
...
At widths of at least 1024px in landscape, six Services chapters alternate the illustration side. Each chapter has a minimum height of `max(330px, (100svh - measured header height) / 2)`, aiming for roughly two chapters per available screen while allowing content to grow. The header is measured with `ResizeObserver`; the initial fallback is 85px. The artwork column is exactly the artwork width, `clamp(224px, 18vw, 440px)`, beside a flexible copy column with a `clamp(32px, 3vw, 64px)` gap. Reverse rows swap the columns, keeping common outer edges instead of centering small images within oversized tracks. The gold offset frame changes side with the chapter.
