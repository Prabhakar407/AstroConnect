...
- All Services chapters retain their compact spacing while alternating a clearly visible, very light warm-brown wash from the artwork side toward transparency. Landscape artwork and copy sit inside that tinted frame rather than on its edge; reverse rows right-align their copy toward the artwork, while portrait/mobile layouts retain their simpler left-aligned flow and a quieter tint.
...
- The featured testimonial advances every 2 seconds until the visitor pauses or manually operates it. The review collection uses 30 client-authorized provisional accounts: six are visible under All Experiences and three under a service filter. One visible card flips to an unseen account every 2.5 seconds, even while the pointer rests over the collection; duplicate visible accounts are prevented, and motion pauses only while offscreen, hidden or reduced by the visitor’s system preference.
...
- Public booking retains its four-step arrangement and incumbent palette. Quantity and total sit in Consultation Details, without customer-facing session-length wording. Time buttons show the appointment start time, while their accessible labels retain the complete appointment range and availability. Calendar dates are 14px and navigation controls are 44px. The date card no longer stretches to add empty height beside the longer Prashna card.
...
- The public booking hero states plainly that consultations booked through the site are online on Google Meet and that links are shared after booking. This concise message sits directly below “Schedule an online consultation”; there is no separate process strip before the form. The confirmed state and customer email both say that the Google Meet link is shared by email. Contact-page FAQs repeat the online format and direct cancellations and refund questions to the studio phone without implying an automatic refund.
