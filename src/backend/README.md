...
Project `fancy-water-31658958`, branch `br-rough-lake-b3ira3cc` (`production`), database `neondb`: migrations **001–014** are applied and immutable. On 2026-09-13, the prepared Neon migration applied 012–014 and deleted its temporary branch. A production readback confirmed all three exact checksums, both new tables, the reconciliation column and `SELECT, INSERT, UPDATE` for the pooled `astro_booking_app` role on only those tables. Use the existing protected operator workflow; never migrate from a request, reset credentials or introduce another database.
...
- `delivery_jobs` stores inquiry, payment-event, Calendar/Meet and participant-email obligations. Provider work is claimed with bounded leases, performed outside the database transaction and completed only by the current lease owner. Repeats reuse saved payload/provider identities. Eight bounded message attempts and separate payment-event limits prevent endless replay; exhausted work remains visible under **Needs attention**. Accepted email is not described as inbox delivery.
