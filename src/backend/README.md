...
Guidance updated 2026-09-11. See [inquiry-dispatch evidence](../../docs/evidence/plan-001/2026-09-11-inquiry-dispatch.md), the [helper runbook](../../workers/inquiry-delivery/README.md), then [Plan 1's resume checkpoint](../../docs/PLAN-001-booking-inquiries-and-client-calendar.md). Earlier reports remain dated evidence, not new real-provider proof.

## What runs now

Local entrypoint: `src.backend.main:app`. Native Vercel candidate: `api/index.py` exports that same ASGI app; framework remains Vite, with API routing before the page fallback. This configuration is prepared, not hosted-verified. It retains FastAPI and PostgreSQL, not local JSON files. Missing database/email configuration never invents availability or a sent code. Checkout stays disabled even if its button is manipulated.

The private calendar route is `/#/studio/calendar`. Its interface and protected API exist; Google ID/secret saved is user-reported, not a successful client login. Inquiry processing, post-commit queue publication, fixed-destination helper and independent scheduled recovery are locally checked. The helper is not deployed. Mail quotas, signed provider observations, private inquiry/attention views, real delivery, payments and meetings remain open. Do not release this as a completed booking system.
...
Project `fancy-water-31658958`, branch `br-rough-lake-b3ira3cc` (`production`), database `neondb`: migrations **001–007** applied and immutable. Migration 006 extends dispatch fields on the existing table; restricted pooled reads/UPDATE grants and TLS reverified 2026-09-11, zero records. Use the existing protected operator file and direct maintenance connection; never migrate from a request, reset credentials or introduce another database. Runtime retains the pooled `astro_booking_app` role.
...
| `ASTRO_DELIVERY_SECRET` | Server-only helper credential, at least 32 characters; required solely by the internal inquiry delivery route. Not a feature switch or customer/admin credential. Configure with the upcoming Cloudflare helper, not in the browser. |
...
Checkout is not implemented yet; its current unavailable response is a temporary placeholder, not a permanent manual switch. P1-08 replaces that route with the real payment flow, removes the booking-code placeholder restriction and derives the website's booking availability from the actual working integration. P1-09 connects the UI; P1-10/P1-13 verify the journey and release. Do not add another enable flag or a stale-health timeout that can keep a repaired system switched off.
...
- New holds require at least 30 minutes' notice using the database clock; exactly 30 minutes is allowed. Availability uses the same cutoff. Holds expire after at most ten minutes; a receipt-authorized unchanged retry retains its frozen price/deadline. One verified email cannot create simultaneous active holds. Price/slot/birth validation failures preserve unconsumed verification proof. These remain internal checkout primitives, not a public payment route or Razorpay proof.
...
- `payments` retains accepted/review references. Phone cancellation frees the slot and preserves the booking/payment record. It does not automatically refund.
- `inquiries` stores Contact/Home/Prashna inquiries. Verification and durable save precede success. UUID plus independent 256-bit receipt secret authorizes retry/status; only a context-bound digest and expiry enter the database. Inquiry access lasts 24 hours; booking receipt access ends no later than appointment end +24 hours. `POST /api/inquiry-status` and `/api/booking-status` require `X-Astro-Receipt`, have a shared 60/minute per-observed-IP limit and return limited summaries, never birth/contact details. Actual hosted IP trust still needs proof. Browser storage retains identifiers only; cross-device verified recovery and full meeting/payment state are unfinished.
- `delivery_jobs` stores two inquiry messages atomically, one per customer/client; booking job semantics remain unchanged. `inquiry_delivery.py` claims one due inquiry job, freezes its email payload, commits, performs one send outside the transaction and records the lease-checked result. Repeats reuse the payload/key. It retains late acceptance evidence without overwriting a newer worker's state. Eight bounded attempts and backoff apply; uncertain sends are not blindly replayed beyond Resend's 24-hour dedupe window. A definitely rejected rate-limit attempt can wait until the next day when no prior uncertain send exists. Accepted is not inbox-delivered. The receipt response now gives only the customer's `email_status` (`pending`, `accepted`, `needs_attention`, or `unavailable`), not the client's private outcome.
...
- `inquiry_dispatch.py` publishes after successful save on both inquiry routes; one bounded call, no delayed in-memory background task. Missing/failed queue configuration leaves the inquiry saved and logs a safe recovery-needed code. Migration 006 records publication claims/results independently of email processing. The helper's 15-minute scheduled event calls `POST /api/internal/recovery/inquiries` with `{run_id}` and `ASTRO_RECOVERY_SECRET`; authorization precedes storage. At most 25 eligible inquiry jobs are republished; a matching 200 identifies application/environment/run/selected/published counts, not customer data. `dispatch_after` ordering and a 90-second claim prevent collisions/starvation; stale publication responses cannot alter newer claims or email results.
- The [Cloudflare helper](../../workers/inquiry-delivery/README.md) is locally built/tested but not deployed. It verifies exact production/job identity and durable terminal state before acknowledgement, rejects redirects/HTML and respects retry times. Signed provider observations, shared mail budgets, private inquiry/attention access and proactive independent alerts remain unfinished. The queue's own retries/retention cannot erase the authoritative SQL jobs; booking/meeting work stays excluded.
...
Latest review ran **142 Python tests without skips** on isolated local PostgreSQL, **12 helper tests** and **11 existing JS tests**. Publication/recovery tests cover both form routes, saved work during failure, lost acceptance/writeback, concurrent/stale publishers, queue expiry, bounded fairness and authentication. Fresh Vite build, Cloudflare dry-run and local runtime startup pass. These are local checks, not hosted or provider proof.
