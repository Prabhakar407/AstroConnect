      ...
      for (const message of batch.messages) {
        ...
        try {
          ...
          if (!object(body) || Object.keys(body).sort().join(',') !== 'environment,job_id,kind' ||
              !['inquiry_received', 'payment_event', 'booking_confirmed', 'booking_cancelled', 'payment_review', 'sheet_booking', 'sheet_inquiry'].includes(body.kind) || body.environment !== ENVIRONMENT ||
          ...
          const path = body.kind === 'payment_event' ? '/api/internal/delivery/payment' :
            body.kind === 'inquiry_received' ? '/api/internal/delivery/inquiry' :
              ['sheet_booking', 'sheet_inquiry'].includes(body.kind) ? '/api/internal/delivery/sheet' : '/api/internal/delivery/booking';
    ...
    async scheduled(controller, env) {
      ...
      try {
        ...
        if (status !== 200 || !identity(data) || data.run_id !== run_id ||
            !Number.isInteger(data.selected) || data.selected < 0 || data.selected > 75 ||
