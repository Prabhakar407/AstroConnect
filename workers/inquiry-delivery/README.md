...
**2026-09-12 permanent-domain continuation:** source now targets https://astroadvicebykundansingh.com after the approved website release. The account, Worker, queue, secrets, schedule and bounded processing are unchanged. Previous preview-target setup below is historical. Deployment outcome is recorded in the current Plan 1 calendar-compact evidence. Resend's existing webhook destination also needs its separate account update; do not delete/recreate it or rotate its signing secret just to change the URL.
