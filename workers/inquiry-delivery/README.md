...
- A 15-minute scheduled event calls `/api/internal/recovery/inquiries` with a **different** credential and random run reference. Python selects at most 25 due jobs, records a short publication claim, commits, publishes once and records acceptance. In the same bounded request it reconciles at most one uncertain order, one signed payment event and due booking work, then returns only safe counts. The Worker deliberately fails the run when that call fails or durable payment/booking attention is non-zero.
...
**Outstanding for the completion candidate:** release the website handlers and helper together; enable the Cloudflare email notification for Worker errors; verify one official-domain Test Mode booking from OTP through payment, Meet and both messages. Do not describe dashboard logs as an alert or local tests as hosted delivery proof.
